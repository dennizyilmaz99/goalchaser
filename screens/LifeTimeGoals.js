...
const storeData = async (value) => {
  try {
    ...
    await AsyncStorage.setItem("listItemsLifeTime", jsonValue);
  } catch (e) {
    console.log(e);
  }
};
...
const getData = async (callback) => {
  try {
    const value = await AsyncStorage.getItem("listItemsLifeTime");
...
const GoalYesterday = () => {
  ...
  const newTodoCounter = () => {
    if (setTodoList) {
      setLifeTimeCounter(LifeTimeCounter + 1);
      return;
    }
  };
  // Here is my removSubmit. I have it in all the screens where you can write your goals so you can remove them and it affects the counter aswell in the HomeScreen.
